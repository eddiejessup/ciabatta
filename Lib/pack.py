...
class MetroRCP(object):
    ...
    def __init__(self, r_0, V_0, R, dr_max, dV_max):
        ...
        self.r_diff_sq = utils.vector_mag_sq(
            self.r[np.newaxis, :] - self.r[:, np.newaxis])
        self.r_diff_sq[np.identity(self.n, dtype=np.bool)] = np.inf

    def U(self):
        if np.any(np.abs(self.r) > self.L / 2.0 - self.R):
            return np.inf
        if np.any(self.r_diff_sq < self.D_sq):
            return np.inf
        return 1.0 / self.pf()
    ...
    def displace_r(self):
        ...
        self.r[self.i] += np.random.uniform(
            -self.dr_max * self.L, self.dr_max * self.L, self.dim)

        self.r_diff_sq[self.i, :] = self.r_diff_sq[
            :, self.i] = utils.vector_mag_sq(self.r[self.i] - self.r)
...
def random_base(n, V, dim, R):
    ...
    for i in range(n):
        ...
        while True:
            ...
            r_diff_sq = utils.vector_mag_sq(
                r[:i + 1, np.newaxis] - r[np.newaxis, :i + 1])
            if i == 0 or r_diff_sq[r_diff_sq > 0.0].min() > (2.0 * R) ** 2:
                break
    return r
...
def random(pf, dim, R, beta_max=1e4, V_0=2.0, dV_max=0.02, dr_max=0.02):
    n = int(round(pf / geom.sphere_volume(R, dim)))
...
def sphere_plot(r, R):
    ...
    if dim == 2:
        ...
        for r in r:
            ...
            ax.plot_surface(
                r[0] + x, r[1] + y, r[2] + z,  rstride=15, cstride=15)
